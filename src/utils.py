...
def validation_PSNR(UHD_Net_reuse, val_data_loader, device, category, save_tag=False):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        with torch.no_grad(): 
            ...
            input_low = lowlight.detach().numpy()
            input_low = np.transpose(input_low, (0, 2, 3, 1))
            input_low = np.ascontiguousarray(input_low)
            num_images = len(input_low)  # 获取 input_low 中包含的数据组数
            """
            dehaze = np.zeros_like(input_low)
            for i in range(num_images):
                image = input_low[i]  # 获取当前组的数据
                image = (image * 255).astype(np.uint8)
                #cv2.imwrite(f"image_{i + 4}.png", cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
                #image = 255 - image  # 处理图像（这里是示例操作）
                h, w = image.shape[0], image.shape[1]
                # 获取numpy对象的数据指针
                frame_data = np.asarray(image, dtype=np.uint8)
                frame_data = frame_data.ctypes.data_as(ctypes.c_char_p)
                # 设置输出数据类型为uint8的指针
                lib.dehazeFromData.restype = ctypes.POINTER(ctypes.c_uint8)
                # 调用so里的dehazeFromData函数
                pointer = lib.dehazeFromData(frame_data, h, w)
                # 从指针指向的地址中读取数据，并转为numpy array
                np_canny = np.array(np.fromiter(pointer, dtype=np.uint8, count=h * w * 3))
                np_canny = np_canny.reshape((h, w, 3))
                lib.release(pointer)
                image = np_canny
                #image = 255 - image  # 处理图像（这里是示例操作）
            ...
            dehaze = dehaze.astype(np.float32) / 255.0
            dehaze = np.transpose(dehaze, (0, 3, 1, 2))
            dehaze = Variable(torch.FloatTensor(torch.from_numpy(dehaze))).cuda()
            """
