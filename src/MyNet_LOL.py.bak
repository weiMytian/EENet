...
from arch_util_NAF import LayerNorm2d
...
class SimpleGate(nn.Module):
    ...
    def forward(self, x):
        ...
        x_0 = x1+x2
        x_1 = x3*x4
        x = self.beta*x_0 + self.gamma*x_1
        return x
...
class NAFBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        self.conv2 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1, stride=1,
                               groups=dw_channel,
                               bias=True)
        self.conv3 = nn.Conv2d(in_channels=dw_channel // 4, out_channels=c, kernel_size=1, padding=0, stride=1,
                               groups=1, bias=True)
        ...
        # SimpleGate
        self.sg = SimpleGate(c=dw_channel)
...
class InteractNet(nn.Module):
    def __init__(self, nc=16):
        ...
        middle_blk_num = 1
        dec_blks = [1, 1]
        self.lownet = NAFNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
